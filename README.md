...
Devfence is Linux-first. It invokes host tools instead of replacing them, and reports the selected backend, workspace mode, network setting, and credential identities before launch. No host SSH identity or GitHub token is passed unless the selected profile configures it.
...
Inspect the resolved policy before starting a session with `devfence plan`. Configuration lives at `${XDG_CONFIG_HOME:-~/.config}/devfence/config.yaml`; session state lives at `${XDG_STATE_HOME:-~/.local/state}/devfence`. See [the configuration reference](docs/configuration.md) and [the security model](docs/security-model.md).
